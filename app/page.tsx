...
export default function Page(){
 ...
 async function load(){
  ...
  const[{data:bd,error:be},{data:cd,error:ce},{data:pd,error:pe}]=await Promise.all([
   ...
   supabase.from('bar_pours').select('id,cocktail_name,made_at').order('made_at',{ascending:false}).limit(1000)
  ])
 ...
 const topPours=useMemo(()=>{const m:Record<string,number>={};for(const p of pours)m[p.cocktail_name]=(m[p.cocktail_name]||0)+1;return Object.entries(m).sort((a,b)=>b[1]-a[1]||nb(a[0],b[0])).slice(0,12)},[pours])
 ...
 return <main>
  ...
  {tab==='stats'&&<section className="content page"><div className="pagehero small"><p className="eyebrow">INNSIKT</p><h1>Hva skjer i baren?</h1><p>Historikken viser de siste {Math.min(pours.length,1000)} registrerte serveringene.</p></div><div className="metrics"><article><GlassWater/><div><b>{pours.length}</b><span>drinker registrert</span></div></article><article><FlaskConical/><div><b>{bottles.length}</b><span>flasker</span></div></article><article><Sparkles/><div><b>{makeable.length}</b><span>kan lages</span></div></article><article><Heart/><div><b>{cocktails.filter(c=>c.is_favorite).length}</b><span>favoritter</span></div></article></div><div className="insightgrid"><article><h2>Mest laget</h2>{topPours.length?topPours.map(([name,n],i)=><p key={name}><span>#{i+1} {name}</span><b>{n}</b></p>):<p>Ingen historikk ennå.</p>}</article><article><h2>Sist laget</h2>{pours[0]?<p><span>{pours[0].cocktail_name}</span><small>{new Date(pours[0].made_at).toLocaleString('nb-NO')}</small></p>:<p>Ingen historikk ennå.</p>}</article><article style={{gridColumn:'1 / -1'}}><h2>Historikk</h2>{pours.slice(0,historyLimit).map(p=><p key={p.id}><span>{p.cocktail_name}</span><small>{new Date(p.made_at).toLocaleString('nb-NO')}</small></p>)}{historyLimit<pours.length&&<button className="loadmore" onClick={()=>setHistoryLimit(v=>Math.min(v+50,pours.length))}>Vis 50 flere ({pours.length-historyLimit} igjen)</button>}</article></div></section>}
