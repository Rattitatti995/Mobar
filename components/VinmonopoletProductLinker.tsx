 ...
 useEffect(()=>{
  ...
  async function chooseProduct(form:HTMLFormElement,p:VinoProduct,panel:HTMLElement){
   ...
   const{error}=await supabase.from('barcode_product_cache').insert({
    ...
    product_name:p.name||p.brand||p.ingredient||'Vinmonopolvare',
   ...
   if(error&&error.code!=='23505'){
    setStatus(form,`Varen ble funnet, men MoBar klarte ikke å lære strekkoden: ${error.message}`,'warn')
   }else{
   ...
   const searchArea=panel.querySelector<HTMLElement>('.vino-search-area')
  ...
  async function runSearch(form:HTMLFormElement,panel:HTMLElement){
   ...
   results.innerHTML='<span class="vino-empty">Søker i Vinmonopolet…</span>'
   ...
   if(error||!data?.ok){
    ...
    msg.className='vino-empty'
    msg.textContent=`Vinmonopolet-søket feilet: ${error?.message||data?.error||'ukjent feil'}`
   ...
   results.innerHTML=''
   if(!products.length){
    const msg=document.createElement('span');msg.className='vino-empty';msg.textContent='Ingen treff. Prøv et kortere produktnavn.';results.appendChild(msg);return
   }
   for(const p of products){
    const option=document.createElement('button')
    option.type='button'
    option.className='vino-result'
    const name=document.createElement('strong')
    name.textContent=p.name||p.brand||'Ukjent Vinmonopolvare'
    const meta=document.createElement('span')
    meta.textContent=productMeta(p)||'Vinmonopolet'
    option.append(name,meta)
    option.onclick=()=>chooseProduct(form,p,panel)
  ...
  function ensurePanel(form:HTMLFormElement){
   ...
   if(!panel){
    ...
    panel.className='vinmonopolet-linker'
    panel.hidden=true
    ...
    status.after(panel)
    const open=panel.querySelector<HTMLButtonElement>('.vino-open')!
    const area=panel.querySelector<HTMLElement>('.vino-search-area')!
    const query=panel.querySelector<HTMLInputElement>('.vino-query')!
    const search=panel.querySelector<HTMLButtonElement>('.vino-do-search')!
    open.onclick=()=>{area.hidden=!area.hidden;if(!area.hidden)window.setTimeout(()=>query.focus(),0)}
    search.onclick=()=>runSearch(form,panel!)
    query.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();runSearch(form,panel!)}})
   }
   ...
   const found=/Vare funnet fra|Koblet strekkoden/i.test(text)
   if(panel.dataset.linked==='true')panel.hidden=false
   else panel.hidden=barcode.length<6||found||!needsOfficial
  }
